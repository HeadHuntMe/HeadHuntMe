 ...
 */
    add_filter('manage_edit-freelancer_columns', 'freelancer_columns_id');
    add_action('manage_freelancer_posts_custom_column', 'freelancer_custom_columns', 5, 2);


    function freelancer_columns_id($defaults){
        unset($defaults['date']);

        $defaults['display_name'] =  __('Display Name', 'exertio_framework');
        $defaults['wallet_amount'] =  __('Wallet', 'exertio_framework');
        return $defaults;

    }
    function freelancer_custom_columns($column_name, $id){
 ...
 // Add the custom columns to the freelancer post type:
...
// Add the data to the custom columns for the freelancer post type:
...
function custom_freelancer_column( $column, $post_id ) {
	...
	function freelancer_save_post_class_meta( $post_id, $post ) {
	  ...
	  if ( !current_user_can( $post_type->cap->edit_post, $post_id ) )
		...
		if(isset($_POST['freelancer_portfolio_link']))
		{
			update_post_meta( $post_id, '_freelancer_portfolio_link', $_POST['freelancer_portfolio_link']);
