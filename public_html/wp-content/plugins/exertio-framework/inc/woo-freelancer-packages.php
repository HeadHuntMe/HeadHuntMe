...
if( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) 
{
	...
	function freelancer_packages_post_class_meta_box( $post ) { ?>
	  ...
	  <?php wp_nonce_field( basename( __FILE__ ), 'freelancer_package_class_nonce' ); 
        ...
        <div class="custom-row">
            <div class="col-3"><label><?php echo __( "Service Bump up", 'exertio_framework' ); ?></label></div>
            <div class="col-3">
                <?php
                $freelancer_listing_bump ='';
                $freelancer_listing_bump = get_post_meta($post_id, '_freelancer_listing_bump', true);
                ?>
                <input type="number" name="freelancer_listing_bump" value="<?php echo $freelancer_listing_bump; ?>" >
	...
	function freelancer_packages_save_post_class_meta( $post_id, $post ) {
	  ...
	  if ( !current_user_can( $post_type->cap->edit_post, $post_id ) )
        ...
        if(isset($_POST['freelancer_listing_bump']))
        {
            update_post_meta( $post_id, '_freelancer_listing_bump', $_POST['freelancer_listing_bump']);
        }
		...
		function exertio_freelancer_order_status_completed( $order_id )
		{
			...
			foreach ( $items as $item )
			{
				...
				if($prduct_type == 'freelancer-packages') 
				{
                    ...
                    $freelancer_listing_bump = get_post_meta($product_id, '_freelancer_listing_bump', true);
                    ...
                    $ext_freelancer_listing_renew = get_post_meta( $freelancer_id, '_freelancer_listing_bump', true );
                    if($ext_freelancer_listing_renew == -1 || $freelancer_listing_bump == -1)
                    {
                        $freelancer_listing_bump = -1;
                    }
                    else if(isset($ext_freelancer_listing_renew) && $ext_freelancer_listing_renew > 0)
                    {
                        $freelancer_listing_bump = $ext_freelancer_listing_renew + $freelancer_listing_bump;
                    }
                    ...
                    update_post_meta( $freelancer_id, '_freelancer_listing_bump', $freelancer_listing_bump);
